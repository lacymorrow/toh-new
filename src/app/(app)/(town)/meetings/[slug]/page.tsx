...
import { getMeetings, getMeetingBySlug } from "@/lib/payload/town-data";
...
export async function generateMetadata({ params }: MeetingPageProps): Promise<Metadata> {
	...
	const meetingDate = new Date((meeting as any).meetingDate).toLocaleDateString();
...
export default async function MeetingPage({ params }: MeetingPageProps) {
	...
	return <MeetingDetail meeting={meeting as any} />;
}

// Generate static paths for popular meetings
export async function generateStaticParams() {
	const result = await getMeetings({ limit: 10 });

	return result.docs.map((meeting) => ({
		slug: meeting.slug,
	}));
}
